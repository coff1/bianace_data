...
def get_long_short_ratio_history(symbol='BTCUSDT', period='5m', limit=500):
    """
    拉取币安合约多空持仓人数比历史数据（从最新往前追溯30天）
    ...
    print(f"开始拉取 {symbol} 多空持仓人数比历史数据...")
    print(f"时间周期: {period}")
...
def save_to_csv(data, filename='BTC多空持仓人数比.csv'):
    """
    将数据保存为CSV文件
...
if __name__ == "__main__":
    # 拉取数据（使用5分钟周期，最大精确度）
    data = get_long_short_ratio_history(symbol='BTCUSDT', period='5m', limit=500)

    # 保存为CSV
    save_to_csv(data, 'BTC多空持仓人数比.csv')
