...
def get_open_interest_history(symbol='BTCUSDT', period='5m', limit=500):
    """
    拉取币安合约持仓量历史数据（从最新往前追溯30天）
...
def save_to_csv(data, filename='BTC合约持仓量历史.csv'):
    """
    将数据保存为CSV文件
    ...
    # 重新排列列顺序
    columns = ['datetime', 'symbol', 'sumOpenInterest', 'sumOpenInterestValue',
    ...
    print(df.head())
    print("\n统计信息:")
    print(df[['sumOpenInterest', 'sumOpenInterestValue']].describe())

if __name__ == "__main__":
    # 拉取数据（使用5分钟周期，最大精确度）
    data = get_open_interest_history(symbol='BTCUSDT', period='5m', limit=500)

    # 保存为CSV
    save_to_csv(data, 'BTC合约持仓量历史.csv')
