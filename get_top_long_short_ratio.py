...
def get_top_long_short_ratio(symbol='BTCUSDT', period='5m', limit=500):
    """
    拉取币安大户持仓量多空比历史数据（从最新往前追溯30天）
    ...
    print(f"开始拉取 {symbol} 大户持仓量多空比历史数据...")
    print(f"时间周期: {period}")
...
def save_to_csv(data, filename='BTC大户持仓量多空比.csv'):
    """
    将数据保存为CSV文件
...
if __name__ == "__main__":
    # 拉取数据（使用5分钟周期，最大精确度）
    data = get_top_long_short_ratio(symbol='BTCUSDT', period='5m', limit=500)

    # 保存为CSV（以数据名称命名）
    save_to_csv(data, 'BTC大户持仓量多空比.csv')
